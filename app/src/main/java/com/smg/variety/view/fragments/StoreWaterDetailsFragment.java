...
public class StoreWaterDetailsFragment extends BaseFragment {
    ...
    public static StoreWaterDetailsFragment newInstance(int type) {
        StoreWaterDetailsFragment fragment = new StoreWaterDetailsFragment();
    ...
    private void loadData() {
        ...
        if (mOrderStatus == 1) {
            map.put("filter[scopeAdd]", "1");
        } else if (mOrderStatus == 2) {
            map.put("filter[scopeReduce]", "1");
        }

        DataManager.getInstance().getSellerslog(new DefaultSingleObserver<HttpResult<List<IncomeDto>>>() {
            @Override
