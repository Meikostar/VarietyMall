...
public class OrderFragmentTg extends BaseFragment implements ToFragmentListener{
    ...
    public OrderFragmentTg(String type) {
        this.types = type;
    }
    ...
    private void initRecyclerView() {
        ...
        mAdapter = new OrderTgAdapter(getActivity());
    ...
    private void loadData() {
        ...
        if (TextUtil.isNotEmpty(star)) {
            if (TextUtil.isNotEmpty(star)) {
                map.put("filter[created_at]", star+ "00:00:00" + ":::" + end+ "00:00:00");
            } else {
                map.put("filter[created_at]", end + "00:00:00"+ ":::");
            }

        } else {
            if (TextUtil.isNotEmpty(end)) {
                map.put("filter[created_at]", ":::" + end+ "00:00:00");
            }
        }
        map.put("page", mPage + "");
        //        map.put("api_nested_no_data", "1");
        DataManager.getInstance().getPromoteOrders(new DefaultSingleObserver<HttpResult<List<MyOrderDto>>>() {
            @Override
