...
//                });
    ...
    public static void install(Context context) {
        ...
        File file = new File(
                Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS) + "/bbsc/App"
                , "bbsc" + versionName + ".apk");
...
//        context.startActivity(intent);
        if (Build.VERSION.SDK_INT >= 24) { //判读版本是否在7.0以上
            ...
            Uri apkUri =
                    FileProvider.getUriForFile(context, "com.smg.variety.FileProvider", file);
    ...
    private void startDownload(String downUrl) {
        ...
        //设置下载存放的文件夹和文件名字
        request.setDestinationInExternalPublicDir(Environment.DIRECTORY_DOWNLOADS + "/bbsc/App", "bbsc" + versionName + ".apk");
        //设置下载时或者下载完成时，通知栏是否显示
        request.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED);
        request.setTitle("下载百变商城新版本");
        //执行下载，并返回任务唯一id
