        ...
        , ImageFloderPop.onFloderItemClickListener, AbsListView.OnScrollListener
{
    ...
    public static final String READ_PLUGIN_MESSAGE_SEND_IMAGE = "read_plugin_message_send_image";//发送阅后即焚图片
...
//            ImagePagerActivity.start(this, dataList, p, mOptions, ImageContants.REQUEST_CODE_DETAIL);
    ...
    private void returnSingleImage(ImageBean imageBean)
    {
        ...
        sendReadMessageImageBroadcast(list);
        finish();
    }
    ...
    private void returnAllSelectedImages()
    {
        ...
        setResult(RESULT_OK, intent);
        sendReadMessageImageBroadcast(resultList);
        finish();
    }
     ...
     * 阅后即焚图片
     */
    private void sendReadMessageImageBroadcast(List<ImageBean> lists) {
        if (null != lists && !lists.isEmpty()) {
            List<String> items = new ArrayList<>();
            for (ImageBean imageBean : lists) {
                items.add(imageBean.getImagePath());
            }
            BroadcastManager.getInstance(this).sendBroadcast(READ_PLUGIN_MESSAGE_SEND_IMAGE, items);
        }
    }
